...
export class ClientAnalyticsEventLogStore {
  ...
  initialize () {
    return this._ca.createCollection(CA_COLLECTION)
      .then(() => this._ca.registerQuery(CA_QUERY_ALL, collection => {
        return collection.chain()
          .simplesort('recordTime', false)
          .simplesort('sequenceNumber', false)
          .data()
      }))
      .then(() => this._ca.registerQuery(CA_QUERY_LATEST, collection => {
